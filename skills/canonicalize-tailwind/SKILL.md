...
2. Canonicalize class strings with `bunx @tailwindcss/cli canonicalize`.
3. Apply changed class strings back to the source.
4. Run the project's formatter or relevant checks when available.

## Commands

- Use `bunx @tailwindcss/cli canonicalize` to clean up Tailwind class lists — collapses shorthands (`mt-2 mr-2 mb-2 ml-2` → `m-2`), resolves overrides (`py-3 p-1 px-3` → `p-3`), canonicalizes arbitrary values to named utilities, and sorts classes; pass `--css path/to/input.css` if the project uses a custom CSS entry file

  Single class string:

  ```sh
  bunx @tailwindcss/cli canonicalize "mt-2 mr-2 mb-2 ml-2"
  # m-2
  ```
  ...
  ```sh
  bunx @tailwindcss/cli canonicalize "py-3 p-1 px-3" "mt-2 mr-2 mb-2 ml-2"
  # p-3
  # m-2
  ```
  ...
  ```sh
  echo "py-3 p-1 px-3\nmt-2 mr-2 mb-2 ml-2" | bunx @tailwindcss/cli canonicalize
  # p-3
  # m-2
  ```
  ...
  ```sh
  bunx @tailwindcss/cli canonicalize --format json "py-3 p-1 px-3"
  ...
  ```sh
  bunx @tailwindcss/cli canonicalize --stream
  ```
